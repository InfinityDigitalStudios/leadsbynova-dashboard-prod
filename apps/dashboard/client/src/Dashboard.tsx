...
export default function Dashboard() {
  ...
  // Get configuration for dynamic agents
  ...
  // Note: Agent names now come from allAgents array instead of separate config values
  ...
  const getLeadsForTab = (tab: string) => {
    ...
    switch (tab) {
      ...
      // Handle dynamic agent tabs
      default:
        // Check if this is an agent tab (agent1, agent2, agent3, etc.)
        const agentMatch = tab.match(/^agent(\d+)$/);
        if (agentMatch) {
          const agentIndex = parseInt(agentMatch[1]) - 1; // Convert to 0-based index
        ...
        // Default case for any other tabs
        return allLeads.filter(lead => lead.archived === "false" && lead.deletedAt === null);
    }
  };
  ...
  // Agent leads are now calculated dynamically in individual components
  ...
  const createLeadMutation = useMutation({
    mutationFn: async (leadData: AddLeadFormData & { section: string }) => {
      let assignedTo = null;
      // Check against all agents dynamically
      const matchingAgent = allAgents.find(agent => agent.name === leadData.section);
      if (matchingAgent) {
        assignedTo = matchingAgent.name;
      }
  ...
  const agentOptions = [
    ...
    ...allAgents.map((agent, index) => ({
      id: `agent${index + 1}`,
      label: agent.name,
      count: activeLeads.filter(lead => lead.assignedTo === agent.name).length
    })),
  ];
  ...
  const renderWorkspaceContent = () => {
    ...
    switch (activeTab) {
        ...
        return (
          <div className="space-y-6">
            ...
            <div className="lg:hidden space-y-6">
              {/* Daily Schedule */}
              <div className="bg-gradient-to-br from-slate-900 via-blue-900 to-slate-900 rounded-lg border border-blue-400/30 overflow-hidden w-full shadow-lg">
                                    ...
                                    {(() => {
                                      ...
                                      return singleHourEvents.map((event) => (
                                        ...
                                        >
                                          ...
                                          <div className="text-white/90 truncate text-sm mt-1">
                                            📞 {getConfigValue('primary_phone', '(000) 000-0000')}
                                          </div>
                                        </div>
                                      ));
                                    })()}
              ...
              <div className="bg-gradient-to-br from-slate-900 via-blue-900 to-slate-900 rounded-lg border border-blue-400/30 overflow-hidden w-full shadow-lg">
                ...
                <div className="h-96 overflow-y-auto p-4">
                  <div className="space-y-4">
                    <div>
                      <Label className="text-white/70">Quick Notes</Label>
                      <Textarea
                        value={overviewNotes}
                        ...
                        rows={8}
                        data-testid="textarea-overview-notes"
                      />
                    </div>
                    
                    <div className="text-center py-8">
                      <p className="text-white/50 italic">No notes yet. Click "Add Note" to create your first note.</p>
                    </div>
                  </div>
                </div>
              </div>
            ...
            {/* Desktop Layout - Side by side with doubled height */}
            <div className="hidden lg:block">
              <div className="grid grid-cols-2 gap-6">
              {/* Daily Schedule - Desktop */}
              <div className="bg-gradient-to-br from-slate-900 via-blue-900 to-slate-900 rounded-lg border border-blue-400/30 overflow-hidden shadow-lg">
                <div className="bg-gradient-to-r from-slate-800/80 via-blue-800/60 to-slate-800/80 p-4 border-b border-blue-400/30">
                  <div className="space-y-3">
                    {/* Top row: Centered title */}
                    <div className="text-center">
        ...
        return (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              {/* Dynamic Agent Cards */}
              {allAgents.map((agent, index) => {
                // Calculate leads for this specific agent dynamically
                const agentLeads = activeLeads.filter(lead => lead.assignedTo === agent.name);
                const agentTabId = `agent${index + 1}`;
                      ...
                      <div className="space-y-4">
                        ...
                        {agentLeads.length > 1 && (
                          <Button 
                            ...
                            onClick={() => setActiveTab(agentTabId)}
                          >
      ...
      case "agent1":
      case "agent2":
        // Get agent name dynamically from allAgents
        const agentMatch = activeTab.match(/^agent(\d+)$/);
        const agentIndex = agentMatch ? parseInt(agentMatch[1]) - 1 : 0;
        const agentName = allAgents[agentIndex]?.name || "Unknown Agent";
        const agentLeads = getLeadsForTab(activeTab);
        const foundAgent = allAgents.find(a => a.name === agentName);
        const agentImage = foundAgent?.headshotUrl || 'data:image/svg+xml,<svg xmlns="http://www.w3.org/2000/svg" width="48" height="48" viewBox="0 0 48 48"><rect width="48" height="48" fill="%23e5e7eb"/><text x="24" y="28" font-family="Arial" font-size="10" text-anchor="middle" fill="%23374151">Agent</text></svg>';
          ...
          <ScrollArea className="flex-1">
            <div className="p-4 space-y-2">
              {navItems.map((item) => {
                ...
                const isActive = activeTab === item.id || (item.isDropdown && (activeTab === "agent1" || activeTab === "agent2"));
